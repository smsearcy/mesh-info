...
from pymeshmap.parser import SystemInfo
...
@main.command(help="Prints information about the nodes and links in the network")
@click.argument("hostname", default="localnode.local.mesh")
@click.option("-v", "--verbose", count=True)
@click.option("--save-errors", is_flag=True)
def network_report(hostname: str, verbose: int, save_errors: bool):
    """Prints information about the nodes and links in the network.

    """

    log_level = {0: "WARN", 1: "INFO", 2: "DEBUG", 3: "TRACE"}.get(verbose, "WARN")
    logger.remove()
    ...
    nodes = asyncio.run(
        crawler.network_nodes(hostname, save_errors=save_errors), debug=async_debug
    )
...
def pprint_node(node: SystemInfo):
    ...
    # TODO: add check for current firmware
    ...
    if node.load_averages:
        ...
        click.echo(
            f"1 min: {node.load_averages[0]}; "
            f"5 min: {node.load_averages[1]}; "
            f"15 min: {node.load_averages[2]}"
        )
    else:
        click.secho("Not Available", fg="yellow")
