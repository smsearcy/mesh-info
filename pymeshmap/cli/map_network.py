...
from .. import models
from ..models import MeshLink, MeshNode, ProcessInfo, ProcessType
...
def main(settings: Dict, hostname: str, verbose: int, dry_run: bool):
    ...
    # TODO: this really only needs to run if not a dry run
    with models.session_scope(session_factory, dry_run) as dbsession:
        # ensure this is the only mapping process running
        process = dbsession.query(ProcessInfo).get(ProcessType.NODES)
        if process is None:
            process = ProcessInfo(type=ProcessType.NODES)
            dbsession.add(process)
        elif process.currently_running:
            raise click.ClickException("Another mapping process is currently running.")
        process.currently_running = True
        process.last_ran = datetime.now()
...
def save_nodes(nodes: List[SystemInfo], dbsession: Session):
    ...
    *Attention:* This does not currently implement the "removed nodes" of
    ...
    for node in nodes:
        ...
        results: List[MeshNode] = dbsession.query(MeshNode).filter(
            or_(
                MeshNode.wlan_ip == node.wifi_ip_address,
                MeshNode.wlan_mac_address == node.wifi_mac_address,
                MeshNode.name == node.node_name,
            )
        ).all()
        if len(results) > 1:
            count["issues"] += 1
            model = fix_multiple_entries(dbsession, results)
        else:
            model = results[0]
        ...
        if model is None:
            ...
            model = MeshNode(
                ...
                name=node.node_name,
                lan_ip=node.lan_ip_address,
                last_seen=datetime.now(),  # timezone?
                up_time=node.up_time,
                load_average=node.load_averages,
                model=node.model,
                ...
                tunnel_installed="true" if node.tunnel_installed else "false",
                active_tunnel_count=str(node.active_tunnel_count),
            )
            dbsession.add(model)
        else:
            ...
            model.lan_ip = node.lan_ip_address
            ...
            model.tunnel_installed = "true" if node.tunnel_installed else "false"
            model.active_tunnel_count = str(node.active_tunnel_count)

            if not model.fixed_location:
                model.latitude = node.latitude
                model.longitude = node.longitude
            else:
                count["skipped fixed location"] += 1

    logger.success("Nodes written to database: {}", dict(count))
    process = dbsession.query(ProcessInfo).get(ProcessType.NODES)
    process.table_last_update = datetime.now()
    process.record_count = count["total"]
    return


def fix_multiple_entries(dbsession: Session, rows: List[MeshNode]) -> MeshNode:
...
def save_links(links: List[LinkInfo], dbsession: Session):
    ...
    for link in links:
        count["total"] += 1
        source: MeshNode = dbsession.query(MeshNode).get(link.source)
        destination: MeshNode = dbsession.query(MeshNode).get(link.destination)
        model = MeshLink(
            source=source.wlan_ip, destination=destination.wlan_ip, cost=link.cost
        )
    ...
    process = ProcessInfo(
        type=ProcessType.LINKS,
        record_count=count["total"],
        table_last_update=datetime.now(),
    )
    dbsession.merge(process)
